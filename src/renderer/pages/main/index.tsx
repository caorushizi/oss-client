...
const Index: React.FC = () => {
  ...
  const tabChange = async (page: Page, bucket: string) => {
    try {
      ...
      if (bucket && bucket !== activeBucket) {
        ...
        const resp = await window.electron.switchBucket(bucket);
  ...
  const onAppSwitch = async (app?: AppStore) => {
    try {
      if (app) {
        // 1、将上下文信息修改为新的 app
        const active = await window.electron.initOss(app._id);
        setActiveApp(active);
  ...
  useEffect(() => {
    ...
    window.electron.onIpcEvent("to-setting", toSetting);
    window.electron.onIpcEvent("transfer-finish", playAudio);
    window.addEventListener("resize", throttleFn);

    return () => {
      window.electron.removeIpcEvent("to-setting", toSetting);
      window.electron.removeIpcEvent("transfer-finish", playAudio);
