...
import { Direction } from "renderer/helper/enums";
import { hiddenTextFilter } from "renderer/helper/filters";
import { debounce, deepEqual } from "renderer/helper/utils";
...
const Services = ({ activeApp, onAppSwitch }: PropTypes) => {
  ...
  const onBucketUpdate = async (store: AppStore) => {
    try {
      ...
      // 检查新的 ak，sk 是否匹配
      await window.electron.getBuckets({ type, ak, sk });
      // 修改数据库中的字段
      setLoading(true);
      await window.electron.updateApp(combineStore);
      // 获取其新的 bucket 列表
      const allApps = await window.electron.getAppsChannel();
      setApps(allApps);
      // 选择当前修改的 app，并且选中
  ...
  const onBucketDelete = async (app: AppStore) => {
    try {
      // 1、弹窗提示
      await window.electron.showConfirm({
        title: "删除",
        message: "确定要删除该应用吗？",
      });
      setLoading(true);
      // 2、点击确定开始删除 app
      const id = app._id;
      await window.electron.deleteApp(id);
      const allApps = await window.electron.getAppsChannel();
      // 判断数据库中是否还有数据
      setApps(allApps);
  ...
  const onBucketAdd = async (values: AddForm) => {
    try {
      ...
      const buckets = await window.electron.getBuckets({ type, ak, sk });
      // 2、获取存储区域，并保存到数据库
      // 3、将 app 名称、ak、sk和服务商名称添加到数据库
      const app = await window.electron.addApp(name, type, ak, sk);
  ...
  const switchApp = async (id: string) => {
    try {
      ...
      // 4、设置默认选中的 appid
      await window.electron.changeSetting("currentAppId", id);
    } catch (e) {
      message.error(e.message);
    }
  };
