...
import { Platform } from "renderer/helper/enums";
import { getPlatform } from "renderer/helper/utils";
...
const Setting = () => {
  ...
  const onSettingChange = async (key: string, value: any) => {
    try {
      setLoading(true);
      await window.electron.changeSetting(key, value);
  ...
  const initState = async () => {
    try {
      const config1 = await window.electron.getConfig();
      setConfig(config1);
    } catch (e) {
      message.error(e.message);
    }
