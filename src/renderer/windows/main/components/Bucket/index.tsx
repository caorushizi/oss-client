...
import { KeyCode, Layout } from "renderer/helper/enums";
...
const Bucket: React.FC<PropTypes> = ({ bucketMeta }) => {
  ...
  const onRefreshBucket = async () => {
    setLoading(true);
    selection.clear();
    const resp = await window.electron.refreshBucket(true);
  ...
  const handleDelete = async (item?: Item) => {
    try {
      ...
      const files = getOperationFiles(item);
      await window.electron.deleteFiles(files.map((i) => i.webkitRelativePath));
  ...
  const onFileContextMenu = (event: MouseEvent<HTMLElement>, item: VFile) => {
    ...
    const menu = remote.Menu.buildFromTemplate([
      ...
      { type: "separator" },
      {
        label: "复制链接",
        click: async () => {
          const url = await window.electron.getFileUrl(item.webkitRelativePath);
  ...
  useEffect(() => {
    window.electron.onIpcEvent("upload-finish", onUploadFinish);

    return () => {
      window.electron.removeIpcEvent("upload-finish", onUploadFinish);
    };
  }, [bucketMeta]);
